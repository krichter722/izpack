...
/**
 * IzPack will be able to support different compression methods for the packs
 * included in the installation jar file. For this a jar output stream will be
 * needed with which the info data (size, CRC) can be written after the
 * compressed data. This is not possible with the standard class
 * java.util.jar.JarOutputStream. Therefore we create an own class which
 * supports it. Really the hole work will be delegated to the ZipOutputStream
 * from the apache team which solves the problem.
 *
 * @author Klaus Bartz
 */
public class JarOutputStream extends org.apache.tools.zip.ZipOutputStream
{
    ...
    /**
     * Creates a new <code>JarOutputStream</code> with no manifest. Using this
     * constructor it will be NOT possible to write data with compression format
     * STORED to the stream without declare the info data (size, CRC) at
     * <code>putNextEntry</code>.
     *
     ...
     * output stream which will be created from the file argument.
     *
     * @param fout the file object with which the output stream should be
     * created
    ...
    /**
     * Creates a new <code>JarOutputStream</code> with no manifest. Will use
     * random access if possible.
     *
     * @param arg0 the file object with which the output stream should be
     * created
     ...
     * This is a hack for FilterOutputStreams like the CBZip2OutputStream of
     * apache which calls close of the slave via the final method which will be
     * called from the garbage collector.
     *
     ...
     * with the stream if isPreventClose is not true. Else nothing will be done.
     * This is a hack for FilterOutputStreams like the CBZip2OutputStream which
     * calls the close method of the slave at finalizing the class may be
     * triggert by the GC.
     *
    ...
    /**
     * Closes this output stream and releases any system resources associated
     * with the stream also isPreventClose is true. This is a hack for
     * FilterOutputStreams like the CBZip2OutputStream which calls the close
     * method of the slave at finalizing the class may be triggert by the GC.
     *
     * @throws IOException if an I/O error occurs.
     */
