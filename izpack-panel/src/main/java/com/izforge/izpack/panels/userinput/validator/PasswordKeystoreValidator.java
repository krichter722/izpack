...
/**
 * This class based on a simple validator for passwords to demonstrate the
 * implementation of a password validator that cooperates with the password
 * field in the <code>UserInputPanel</code>. Additional validation may be done
 * by utilizing the params added to the password field.
 *
 * @author Elmar Grom
 * @author Jeff Gordon
 */
public class PasswordKeystoreValidator extends AbstractValidator
{
     ...
     * optionally 'keystoreType' (defaults to JKS), 'keystoreAlias' (to check
     * for existence of a key), and 'aliasPassword' (for trying to retrieve the
     * key). An additional parameter 'skipValidation' can be set to 'true' in a
     * checkbox and allow the validator framework to run, but not actually do
     * the validation.
     * <p/>
     ...
     * set in the keystorePassword parameter.
     *
